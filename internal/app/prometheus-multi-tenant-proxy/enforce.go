...
type PromQLEnforcer struct {
	...
	exportedLabelMatchers map[string]*labels.Matcher
	errorOnReplace bool
...
func NewPromQLEnforcer(errorOnReplace bool, metricswhitelist []string, exportedmetrics []string, namespacesRe []string,  ms ...*labels.Matcher) *PromQLEnforcer {
	entries := make(map[string]*labels.Matcher)
	exportedEntries := make(map[string]*labels.Matcher)

	for _, matcher := range ms {
		entries[matcher.Name] = matcher
		exportedEntries[matcher.Name] = &labels.Matcher{Type: matcher.Type, Name: "exported_"+matcher.Name, Value: matcher.Value}
	}

	return &PromQLEnforcer{
		labelMatchers:  entries,
		exportedLabelMatchers: exportedEntries,
		errorOnReplace: errorOnReplace,
		metricswhitelist: metricswhitelist,
		exportedmetrics: exportedmetrics,
		namespacesRe: namespacesRe,
	}
}
...
func (ms PromQLEnforcer) EnforceNode(node parser.Node) error {
	...
	case *parser.MatrixSelector:
		for _, m := range ms.metricswhitelist {
                        if m == node.String() {
                                return nil
                        }
                }
	...
	case *parser.VectorSelector:
		log.Printf("Vector: %v", node.String())
		node := node.String()
...
func (ms PromQLEnforcer) EnforceMatchers(targets []*labels.Matcher) ([]*labels.Matcher, error) {
	...
	exported := false
	matcherAdded := make(map[string]bool)

	for _, target := range targets {
		testName := target.Name
		if strings.HasPrefix(target.Name, "exported_") {
			testName = target.Name[9:len(target.Name)]
		}
		matcher, ok := ms.labelMatchers[testName]
		if !ok {
			for _, exp := range ms.exportedmetrics {
				if exp == target.Value {
					exported = true
					continue
				}
			}
			if(! matcherAdded[target.Name]) {
 				res = append(res, target)
				matcherAdded[target.Name] = true
			}
			continue
		}

		if target.Type == labels.MatchRegexp  {
			for _, n := range ms.namespacesRe {
				if target.Value == n {
					if(! matcherAdded[target.Name]) {
						res = append(res, target)
						matcherAdded[target.Name] = true
		...
		if matcher.Type == labels.MatchRegexp && matcher.Name == testName && target.Type == labels.MatchEqual {
			match, _ := regexp.MatchString(matcher.Value, target.Value) 
			if (!match) {
				continue
			}
		} else {
	...
	if(exported) {
		for _, enforcedMatcher := range ms.exportedLabelMatchers {
			if (!matcherAdded[enforcedMatcher.Name]) {
				res = append(res, enforcedMatcher)
				matcherAdded[enforcedMatcher.Name] = true
			}
		}
	} else {
